...
trait PageTrait
{
     ...
     * $this->pageSize
     */
    public function pageInit($page, $pageSize)
    {
        $this->page = $page;
    ...
    public function getPaginator($page, $pageSize){
        ...
        $info = $model->orderBy('id','desc')->offset($this->begin)->limit($pageSize)->get();
//        $total = ceil($model->count() / $pageSize);   //页码总数
        $total = $model->count();  //数据总数
        $data = [
            'data' => $info,
            'total' => $total
        ];
